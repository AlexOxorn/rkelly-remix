...
module RECMA
  class Tokenizer
    ...
    KEYWORDS_THAT_IMPLY_REGEX = KEYWORDS.reject { |k, _v| KEYWORDS_THAT_IMPLY_DIVISION[k] }
    ...
    #   in UTF-8 text.
    #
    WHITESPACE_REGEX = if ''.respond_to?(:encoding)
                         /[[:space:]]+/m
                       else
                         /
                             (
    ...
    def initialize
      ...
      token(:RAW_IDENT, /([_\$A-Za-z][_\$0-9A-Za-z]*)/, word_chars) do |_type, value|
        if KEYWORDS[value]
      ...
      literal_regex = Regexp.new(LITERALS.keys.sort_by do |x|
                                   x.length
                                 end.reverse.map { |x| x.gsub(/([|+*^])/, '\\\\\1').to_s }.join('|'))
      token(:LITERALS, literal_regex, literal_chars) do |_type, value|
        [LITERALS[value], value]
      end

      symbols = ('!'..'/').to_a + (':'..'@').to_a + ('['..'^').to_a + ['`'] + ('{'..'~').to_a
      token(:SINGLE_CHAR, /./, symbols) do |_type, value|
        [value, value]
      end
    end
    ...
    def raw_tokens(string)
      ...
      until scanner.eos?
        ...
        accepting_regexp = followable_by_regex(token) if token.name != :S

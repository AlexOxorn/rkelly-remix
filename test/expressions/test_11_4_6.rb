...
class Expressions_11_4_6_Test < ECMAScriptTestCase
  ...
  1.upto(15) do |x|
    @@sign_tests << [format('%#x', x), x]
    @@sign_tests << [format('%#x', x).gsub(/x/, 'X'), x]
    if format('%#x', x) =~ /[a-f]/
      @@sign_tests << [format('%#x', x).gsub(/([a-f])/) { |m| m.upcase }, x]
      @@sign_tests << [format('%#x', x).upcase, x]
    end
  end

  @@sign_tests.each do |actual, expected|
